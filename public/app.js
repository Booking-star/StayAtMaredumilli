...
function openReel(index) {
  ...
  reelEmbed.innerHTML = `
    ...
    <iframe src="${escapeHtml(embedUrl)}" class="reel-iframe" style="display: none; width: 100%; max-width: 420px; height: 520px; border: 0; border-radius: 12px; background: transparent; overflow: hidden;" scrolling="no" allowtransparency="true" allowfullscreen="true"></iframe>
  `;
  
  reelModal.showModal();

  const loader = reelEmbed.querySelector(".reel-loader");
  const iframe = reelEmbed.querySelector(".reel-iframe");
